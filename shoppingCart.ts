...
let shoppingCart: ProductsInterface[] = [];


// Step 4: Function to add a product to the cart (using arrow function)
const addToCart = (product: ProductsInterface): void => {
...
const calculateTotal = (cart: ProductsInterface[]): string => {
    let total = 0;
...
const displayCart = (cart: ProductsInterface[]): void => {
    if (cart.length === 0) {
